...
# Conservative CPU defaults for shared HPC nodes. These limit native thread
# pools only; they do not change the statistical model or random seed.
for _var in [
...
def sdv_generate(method: str, model_df: pd.DataFrame, n: int, epochs: int, enable_gpu: bool, batch_size: int, verbose: bool, discriminator_lr: float):
    ...
    if method == "tvae":
        synth = TVAESynthesizer(metadata, epochs=epochs, batch_size=batch_size, verbose=verbose, enable_gpu=enable_gpu)
    elif method == "ctgan":
        synth = CTGANSynthesizer(metadata, epochs=epochs, batch_size=batch_size, discriminator_lr=discriminator_lr, verbose=verbose, enable_gpu=enable_gpu)
    elif method == "copulagan":
        synth = CopulaGANSynthesizer(metadata, epochs=epochs, batch_size=batch_size, discriminator_lr=discriminator_lr, verbose=verbose, enable_gpu=enable_gpu)
...
def main() -> None:
    ...
    # memory pressure without changing model parameters.
    from joblib import parallel_config
    with parallel_config(n_jobs=args.joblib_n_jobs):
        if args.method == "arf":
            syn_model = arf_generate(model_df, len(train))
        else:
            syn_model = sdv_generate(
                args.method, model_df, len(train), args.epochs, args.cuda,
                args.batch_size, args.verbose, args.discriminator_lr
            )
