...
def main():
    ...
    pima = root / "pima"
    files = list(pima.rglob("main_results_row.csv"))
    if not files:
        raise SystemExit("No PIMA main_results_row.csv files found")
    ...
    d = pd.DataFrame(rows)
    if not (d["dataset"].astype(str).str.lower() == "pima").all():
        bad = d[d["dataset"].astype(str).str.lower() != "pima"]
        raise SystemExit("Non-PIMA row under PIMA results tree:\n" + bad.to_string())

    print("PIMA primary-row inventory")
    print("=" * 90)
    ...
    if unexpected:
        errors.append(f"Unexpected methods under PIMA main benchmark: {unexpected}")

    if len(d) != 40:
        errors.append(f"Expected exactly 40 PIMA primary rows, found {len(d)}")
    ...
    table = Path(args.tables_out) / "pima_main_table_mean_sd.csv"
    print("\nPIMA FINAL TABLE")
    print("=" * 90)
