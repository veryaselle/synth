#!/usr/bin/env python3
"""Prepare an independent second realization of the benchmark train/test splits.
...
Design
------
- Realization seed: 2026 (predeclared in this script / CLI default).
- PIMA mirrors the primary legacy-compatible scheme by using five independent
  stratified train_test_split calls with seeds 2026..2030.
- Cleveland and CKD mirror the primary scheme by using StratifiedShuffleSplit
  with n_splits=5 and random_state=2026.
...
def main() -> None:
    ...
    p.add_argument("--pima", default=str(REPOSITORY_ROOT / "data/pima.csv"))
    p.add_argument(
        "--cleveland",
        default=str(REPOSITORY_ROOT / "data/Heart_disease_cleveland_new.csv"),
    )
    p.add_argument(
        "--ckd",
        default=str(REPOSITORY_ROOT / "data/kidney_disease_sanitized.csv"),
    )
    ...
    print(f"Second split realization written to: {out_root}")
    print("Validation: PASS")
    print("No second-realization test partition exactly duplicates a primary frozen test partition.")
